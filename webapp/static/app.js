...
$('#run-compare').addEventListener('click', async () => {
  ...
  try {
    ...
    const thru = r.throughput || {};
    const rows = [
      ['Throughput (u/h)', `${fmt2(thru.baseline_mean)} ± ${fmt2(thru.baseline_stdev)}`, `${fmt2(thru.variant_mean)} ± ${fmt2(thru.variant_stdev)}`],
      ['Paired Δ', '—', `${fmt2(thru.paired_delta_mean)} ± ${fmt2(thru.paired_delta_stdev)} (${fmt2(thru.delta_percent)}%)`],
